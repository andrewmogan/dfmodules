...
void
HDF5DataStore::write(const daqdataformats::TriggerRecord& tr)
{
...
void
HDF5DataStore::write(const daqdataformats::TimeSlice& ts)
{
                                   ...
                                   daqdataformats::sequence_number_t sequence_number)
{
  ...
  if (m_file_handle->is_trigger_record_type()) {
    try {
      ...
      if (current_rid == rids.end()) {
        return std::optional<daqdataformats::TriggerRecord>();
      }

      auto next_rid = current_rid++;
      if (next_rid == rids.end()) {
        m_file_index++;
      ...
      return m_file_handle->get_trigger_record(trigger_number, sequence_number);
    } catch (hdf5libs::RecordIDNotFound const& ex) {
      return std::optional<daqdataformats::TriggerRecord>();
    }
  }
...
HDF5DataStore::read_time_slice(daqdataformats::timeslice_number_t timeslice_number)
{
  ...
  if (m_file_handle->is_timeslice_type()) {
    try {
      ...
      auto current_rid = rids.find(ts_rid);
      if (current_rid == rids.end()) {
        return std::optional<daqdataformats::TimeSlice>();
      }
      ...
      return m_file_handle->get_timeslice(timeslice_number);
    } catch (hdf5libs::RecordIDNotFound const& ex) {
      return std::optional<daqdataformats::TimeSlice>();
    }
  }
...
void
HDF5DataStore::prepare_for_run(daqdataformats::run_number_t run_number, bool run_is_for_test_purposes)
{
...
void
HDF5DataStore::finish_with_run(daqdataformats::run_number_t /*run_number*/)
{
  if (m_file_handle.get() != nullptr) {
...
void
HDF5DataStore::generate_opmon_data()
{
...
std::string
HDF5DataStore::get_file_name(daqdataformats::run_number_t run_number)
{
...
bool
HDF5DataStore::increment_file_index_if_needed(size_t size_of_next_write)
{
...
void
HDF5DataStore::open_file_if_needed(const std::string& file_name, unsigned open_flags)
{
...
size_t
HDF5DataStore::get_free_space(const std::string& the_path)
{
  struct statvfs vfs_results;
