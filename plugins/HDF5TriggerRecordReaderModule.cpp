/**
 * @file HDF5TriggerRecordReaderModule.cpp HDF5TriggerRecordReaderModule class implementation
 *
...
#include "HDF5TriggerRecordReaderModule.hpp"
#include "dfmodules/CommonIssues.hpp"
#include "dfmodules/opmon/DataWriter.pb.h"

#include "confmodel/Application.hpp"
#include "confmodel/Session.hpp"
#include "appmodel/HDF5TriggerRecordReaderModule.hpp"
 ...
 */
//#define TRACE_NAME "HDF5TriggerRecordReaderModule"                   // NOLINT This is the default
enum
{
...
HDF5TriggerRecordReaderModule::HDF5TriggerRecordReaderModule(const std::string& name)
  : dunedaq::appfwk::DAQModule(name)
  , m_queue_timeout(100)
  , m_data_storage_is_enabled(true)
  , m_thread(std::bind(&HDF5TriggerRecordReaderModule::do_work, this, std::placeholders::_1))
{
  register_command("conf", &HDF5TriggerRecordReaderModule::do_conf);
  register_command("start", &HDF5TriggerRecordReaderModule::do_start);
  register_command("stop", &HDF5TriggerRecordReaderModule::do_stop);
  register_command("scrap", &HDF5TriggerRecordReaderModule::do_scrap);
}

void
HDF5TriggerRecordReaderModule::init(std::shared_ptr<appfwk::ConfigurationManager> mcfg)
{
  TLOG_DEBUG(TLVL_ENTER_EXIT_METHODS) << get_name() << ": Entering init() method";
  auto mdal = mcfg->get_dal<appmodel::HDF5TriggerRecordReaderModule>(get_name());
  if (!mdal) {
...
void
HDF5TriggerRecordReaderModule::generate_opmon_data() {
...
void
HDF5TriggerRecordReaderModule::do_conf(const data_t&)
{
  ...
  if (m_data_writer.get() == nullptr) {
    throw InvalidHDF5TriggerRecordReaderModule(ERS_HERE, get_name());
  }
...
void
HDF5TriggerRecordReaderModule::do_start(const data_t& payload)
{
  ...
  if (m_data_storage_is_enabled) {
    ...
    if (m_data_writer.get() == nullptr) {
      ...
      ers::fatal(InvalidHDF5TriggerRecordReaderModule(ERS_HERE, get_name()));
    }
  ...
  //											     bind( &HDF5TriggerRecordReaderModule::receive_trigger_record, this, std::placeholders::_1) );
...
void
HDF5TriggerRecordReaderModule::do_stop(const data_t& /*args*/)
{
...
void
HDF5TriggerRecordReaderModule::do_scrap(const data_t& /*payload*/)
{
...
void
HDF5TriggerRecordReaderModule::receive_trigger_record(std::unique_ptr<daqdataformats::TriggerRecord> & trigger_record_ptr)
{
...
void
HDF5TriggerRecordReaderModule::do_work(std::atomic<bool>& running_flag) {
...
DEFINE_DUNE_DAQ_MODULE(dunedaq::dfmodules::HDF5TriggerRecordReaderModule)
